        ...
        {ambassadors.map((ambassador, index) => {
          ...
          return (
            <SwiperSlide key={ambassador.id}>
              ...
              >
                <div
                  className="w-[260px] h-[260px] overflow-hidden shrink-0 will-change-transform transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)]"
                >
                  <img
                    src={ambassador.image_url}
                    alt={ambassador.full_name}
                    className="w-full h-full overflow-hidden shrink-0 will-change-transform transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)]"
                                    onMouseEnter={() => setHoveredIndex(index)}
                onMouseLeave={() => setHoveredIndex(null)}
                                      style={{
                    borderRadius: isHovered
                      ? "100% 100% 12% 100% / 100% 100% 12% 100%"
                      : "100% 12% 100% 100% / 100% 12% 100% 100%",
                  }}
                  />
                </div>
